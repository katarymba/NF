...
from ..models import Order as OrderModel, OrderItem as OrderItemModel, Product, User
...
# Модель для создания заказа из Sever-Fish
class ExtendedOrderCreate(BaseModel):
    user_id: int
    total_amount: float
    items: List[OrderItem]

# Модель для ответа из Sever-Fish
...
# Получить детали заказа (из Sever-Fish)
...
# Создать новый заказ (из Sever-Fish)
@router.post("/", response_model=OrderResponse)
def create_order(order_data: ExtendedOrderCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    # Создаем заказ
...
def get_orders_stats(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
# Получение одного заказа по ID (детальное, из orders_api)
@router.get("/detail/{order_id}", response_model=Order)
def get_order(order_id: int = Path(...), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    try:
...
def update_order_status(order_update: OrderUpdate, order_id: int = Path(...), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    try:
...
def get_orders_by_status(status: str = Path(...), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    try:
