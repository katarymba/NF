...
const CURRENT_DATE = '2025-06-24 17:20:09';
...
const Orders: React.FC = () => {
      ...
      printWindow.document.write(`
        ...
        <body>
          ...
          <div class="section">
            ...
            <p><strong>Имя:</strong> ${order.client_name || order.name || 'Не указано'}</p>
            <p><strong>ID пользователя:</strong> ${order.user_id}</p>
            <p><strong>Адрес доставки:</strong> ${order.delivery_address || 'Не указан'}</p>
            ${order.contact_phone || order.phone ? `<p><strong>Телефон:</strong> ${order.contact_phone || order.phone}</p>` : ''}
          ...
          <div class="section">
            ...
            <p><strong>Сумма заказа:</strong> ${formatPrice(order.total_amount || order.total_price)}</p>
          </div>
    ...
    return orders.filter(order => {
      ...
      if (searchText) {
        ...
        const searchMatch =
          ...
          (order.contact_phone && order.contact_phone.toLowerCase().includes(lowerSearchText)) ||
  ...
  // Колонки для таблицы (убрана колонка "Клиент")
  const columns = [
    {
      title: 'ID',
      dataIndex: 'id',
    ...
    },
    {
      ...
      render: (amount: number, record: Order) => formatPrice(amount || record.total_price || 0)
    },
    {
      title: 'Статус',
      dataIndex: 'status',
  ...
  return (
    <div className="orders-page">
      ...
      {/* Карточки статистики - выровненная версия */}
      <Row gutter={16} style={{ marginBottom: '16px' }}>
        <Col span={4}>
          <Card size="small" className="stat-card" style={{ textAlign: 'center' }}>
      ...
      {/* Панель фильтров - компактная и выровненная версия */}
      ...
      {/* Таблица заказов - выровненная версия */}
        ...
        {orderDetails && (
          <div>
            <Card title="Основная информация" style={{ marginBottom: '16px' }}>
              ...
              <p><strong>Общая сумма:</strong> {formatPrice(orderDetails.total_amount || orderDetails.total_price || 0)}</p>
            </Card>
            
            <Card title="Клиент" style={{ marginBottom: '16px' }}>
              <p><strong>Имя:</strong> {orderDetails.client_name || orderDetails.name || 'Не указано'}</p>
              <p><strong>ID пользователя:</strong> {orderDetails.user_id}</p>
              <p><strong>Телефон:</strong> {orderDetails.contact_phone || orderDetails.phone || 'Не указан'}</p>
            ...
            >
              {orders.map(order => (
                <Option key={order.id} value={order.id}>
                  {`Заказ #${order.id} - ${formatPrice(order.total_amount || order.total_price || 0)} - ${order.client_name || order.name || 'Без имени'}`}
                </Option>
              ))}
            </Select>
          </Form.Item>
